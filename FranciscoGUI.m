function varargout = FranciscoGUI(varargin)
%FRANCISCOGUI M-file for FranciscoGUI.fig
%      FRANCISCOGUI, by itself, creates a new FRANCISCOGUI or raises the existing
%      singleton*.
%
%      H = FRANCISCOGUI returns the handle to a new FRANCISCOGUI or the handle to
%      the existing singleton*.
%
%      FRANCISCOGUI('Property','Value',...) creates a new FRANCISCOGUI using the
%      given property value pairs. Unrecognized properties are passed via
%      varargin to FranciscoGUI_OpeningFcn.  This calling syntax produces a
%      warning when there is an existing singleton*.
%
%      FRANCISCOGUI('CALLBACK') and FRANCISCOGUI('CALLBACK',hObject,...) call the
%      local function named CALLBACK in FRANCISCOGUI.M with the given input
%      arguments.
%
...
% Edit the above text to modify the response to help FranciscoGUI

% Last Modified by GUIDE v2.5 18-Apr-2011 19:18:09

% Begin initialization code - DO NOT EDIT
gui_Singleton = 1;
gui_State = struct('gui_Name',       mfilename, ...
                   'gui_Singleton',  gui_Singleton, ...
                   'gui_OpeningFcn', @FranciscoGUI_OpeningFcn, ...
                   'gui_OutputFcn',  @FranciscoGUI_OutputFcn, ...
                   'gui_LayoutFcn',  [], ...
                   'gui_Callback',   []);
...
% --- Executes just before FranciscoGUI is made visible.
function FranciscoGUI_OpeningFcn(hObject, eventdata, handles, varargin)
...
% Choose default command line output for FranciscoGUI
...
% UIWAIT makes FranciscoGUI wait for user response (see UIRESUME)
% uiwait(handles.figure1);

% --- Outputs from this function are returned to the command line.
function varargout = FranciscoGUI_OutputFcn(hObject, eventdata, handles)
...
c5 = get(handles.checkbox10,'Value');
...
headers = {get(handles.checkbox1,'String') get(handles.checkbox2,'String') get(handles.checkbox3,'String') get(handles.checkbox9,'String') ...
    get(handles.checkbox10,'String') get(handles.checkbox11,'String') get(handles.checkbox12,'String') get(handles.checkbox13,'String')};
...
for j = 1:length(headers)
    if strcmp(headers{j},'PI') == 0
        ...
        if cell2mat(handles.x.knownGene(g,5)) == 1
            plot(tss*ones(1,2),[0 top(j)],'go-',tes*ones(1,2),[0 top(j)],'ro-','LineWidth',2)
        else
            plot(tss*ones(1,2),[0 top(j)],'ro-',tes*ones(1,2),[0 top(j)],'go-','LineWidth',2)
        end

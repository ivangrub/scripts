...
perID = {'HY'};
headers = {'TAF7L_PreDiff' 'TBP_PreDiff' 'PolII_PreDiff' 'TAF7L_PostDiff' 'TBP_PostDiff' 'PolII_PostDiff'};
...
for i = 1
    A = importdata(sprintf('%s_%s_Poisson_Conden.txt',perID{1},headers{headersind(i)}),'\t',1);
    data = A.data;
    text = A.textdata;
    IND = 1;
    ...
    for j = i + 1:length(headersind)
        A = importdata(sprintf('%s_%s_Poisson_Conden.txt',perID{1},headers{headersind(j)}),'\t',1);
        data = A.data;
        text = A.textdata;
        IND = 1;
        for k = 1:length(Fields)

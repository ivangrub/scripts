...
for i = 1:(control-1)
    ...
    fid = fopen(sprintf('%s_%s_Sig.txt',headers{i},comment),'w');
        ...
        for k = 1:length(a)
            ...
            if (sig.Distance(a(k),Win(b(k))) > knownGene{j,6}) && ...
                ...
                SigTable(l,1:16) = {knownGene(j,1) GID(j) knownGene{j,2:7} ...
                    ...
                    sig.Window(a(k),1) sig.Window(a(k),2) sig.Reads(a(k),1)};
            else
                SigTable(l,1:16) = {knownGene(j,1) GID(j) knownGene{j,2:7} ...
    ...
    fid = fopen(sprintf('%s_%s_Conden.txt',headers{i},comment),'w');

...
CondenTable = cell(length(knownGene),19);
...
%     fclose(fid);clear fid
    ...
    fid = fopen(sprintf('%s_%s_relativeto_%s_%s_Conden.txt',person,headers{IPidx(i)},headers{controlidx},comment),'w');
    ...
    for j = 1:length(conden)
        ...
        if (conden(j,3) > knownGene{conden(j,4),6}) && ...
            ...
            CondenTable(n,1:19) = {knownGene(conden(j,4),1) GID(conden(j,4)) knownGene{conden(j,4),2:7} ...
                Orientation(o) Orientation(l) 'Intragenic' conden(j,3) conden(j,5) conden(j,6) ...
                conden(j,1) conden(j,2) conden(j,7) conden(j,8) mean(conden(j,9:10))};
        else
            CondenTable(n,1:19) = {knownGene(conden(j,4),1) GID(conden(j,4)) knownGene{conden(j,4),2:7}...
        ...
        fprintf(fid,sprintf('%s\t%d\t%s\t%s\t%s\t%d\t%d\t%d\t%s\t%s\t%s\t%d\t%d\t%d\t%d\t%d\t%d\t%d\n', ...
            ...
            cell2mat(CondenTable(n,15)),cell2mat(CondenTable(n,16)),cell2mat(CondenTable(n,17)),cell2mat(CondenTable(n,19)),cell2mat(CondenTable(n,18))));
        n = n + 1; 
    end

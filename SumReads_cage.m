...
person = {'Lea'};
headers = {'Cage_MoreData_MacroSpecific' 'Cage_MinData_MacroSpecific'};
...
for i = 1:length(headers)
    ...
    fid = fopen(sprintf('%s_%s.mm9.bw',person{1},headers{i}),'r');
    x = eval(headers{i});
    j = 0;
    while 1
        j = j + 1;
        ...
        [~,~,strand,CHR,st,seq,~,~,~] = strread(tline,'%s%s%c%s%d%s%s%d%s','delimiter','\t');
...
TagCount
save(sprintf('%s_Cage_%s',perID{1},date),headers{:},'headers','len', ...
    'read_length','window','ReadSum','test','perID','-v7.3')

for k = 1:length(headers)
    save(sprintf('%s_%s_SumReads.mm9.mat',perID{1},headers{k}),sprintf('%s',headers{k}))
end

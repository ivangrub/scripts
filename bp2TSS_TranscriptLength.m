...
function avgreads = bp2TSS_TranscriptLength(x,knownGene,read_length,win)
...
for i = 1:length(knownGene)
    ...
    if ~isempty(f)
        ...
        bpres((dist(f(j)):dist(f(j))+read_length-2)+win,k) = bpres((dist(f(j)):dist(f(j))+read_length-2)+win,k)./abs(cell2mat(knownGene(i,7))-cell2mat(knownGene(i,6)));
    end
end

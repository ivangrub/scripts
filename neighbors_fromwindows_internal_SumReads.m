function [Conden Chr_conden TSS_conden] = neighbors_fromwindows_internal_SumReads(len,window_thresh,x,knownGene,chr)
...
for r = 1:length(chr)
    ...
    for f = 1:length(subwin)
            ...
            if a > 1 && (Conden(a,1) <= Conden(a-1,2) && Conden(a,2) >= Conden(a-1,2))
                ...
                if isempty(A) == 0
                    for k = 1:length(A)
                        ...
                        b = find(x.Window(1,subwin) == Conden(a,1));
                        Conden(a,7) = sum(x.Reads(1,subwin(b):subwin(a1)));
        ...
        if a > 1 && (Conden(a,1) <= Conden(a-1,2) && Conden(a,2) >= Conden(a-1,2))
            b = find(x.Window(1,subwin) == Conden(a-1,1));
            A = unique(x.Distance(subwin(b):subwin(a2),[2 4 7 9]));
            l = isnan(A) == 0;
            A = A(l);
            l = sum(l);
            a = a - l;
            if isempty(A) == 0
                for k = 1:length(A)
                    ...
                    b = find(x.Window(1,subwin) == Conden(a,1));
                    Conden(a,7) = sum(x.Reads(1,subwin(b):subwin(a2)));
                    NewMid = mean(Conden(a,1:2),2);
                    if knownGene{A(k),5} == 1
                        start = 6;
                        endit = 7;
            ...
            if isempty(A) == 0
                for k = 1:length(A)
                    Conden(a,1:2) = [x.Window(1,subwin(a1)) x.Window(2,subwin(a2))];
                    Conden(a,7) = sum(x.Reads(1,subwin(a1):subwin(a2)));
                    NewMid = mean(Conden(a,1:2),2);
                    if knownGene{A(k),5} == 1
                        start = 6;
                        endit = 7;

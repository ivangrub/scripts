...
for i = 1:length(headers)
    x = open(sprintf('../ChIP-Seq Data/%s/%s_%s.mm9.mat',person{1},person{2},headers{i}));
...
for i = 1:length(chr);
    ...
    tss1 = 1:win:x-win+1;
    tss2 = win+1:win:x;
    if length(tss1) ~= length(tss2)
        if length(tss1) > length(tss2)
            tss1 = tss1(1:length(tss2));
        else
            tss2 = tss2(1:length(tss1));
        end
    end
    a = length(tss1);
    C = [tss1' tss2'];

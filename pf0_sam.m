function [Xfit,Xbnd,normalization,chip] = pf0(fname, jmp, DNA_len, genome_file, minerr)
...
if nargin<2, jmp=10; elseif ~isnumeric(jmp), jmp=str2double(jmp); end;
if nargin<3, DNA_len=150; elseif ~isnumeric(DNA_len), DNA_len=str2double(DNA_len); end;
if nargin<4, genome_file = 'genome_dm3.mat'; end;
...
if ~exist(mname,'file'),
    global chip ab; [chip, ab] = ReadReads(fname, jump, DNA_len); save(mname,'chip','ab','jump','lens'); clear chip ab; %#ok<NASGU,REDEF>
end
...
rp = round(0.5+param);
PP=[rp'-1,rp'+1]; df = interp1(1:n,fitted,PP,'linear'); df=(df(:,2)-df(:,1))'/2;
...
i = regexp(fname, '\.bed|\.sam'); ab = fname(1:i-1); clear i;
...
while 1 
    ...
    % sam format
    [tag,~,bin,st,~,~,~,~,~,seq,~,~,~,frac_str] = strread(tline,'%s%s%s%d%s%s%s%s%s%s%s%s%s%s','delimiter','\t');
        if ~strcmp(tag{1}(1),'H')
                continue
        end
        [~,CHR,binstart,~] = strread(bin{1},'%s%s%d%d','delimiter','!');
        start = binstart + st;
        [~,f] = strread(frac_str{1},'%s%s','delimiter',' ');
        frac = str2num(strrep(f{1},'XP:f:',''));
    chr = CHR{1};
    ...
    chip.(chr)(pp) = chip.(chr)(pp) + frac; nreads = nreads + 1;
...
for i=1:length(Xbnd),
    ...
    for j=1:length(x.peaks),
        if x.peaks(j)-5*jump > 0 && x.peaks(j)+5*jump < lens.(x.chr)
           fprintf(fid, '%s %d %d %.1f 1 . 0 0 255,192,55\n', x.chr, x.peaks(j)-5*jump, x.peaks(j)+5*jump, x.heights(j));
        else
           fprintf(fid, '%s %d %d %.1f 1 . 0 0 255,192,55\n', x.chr, x.peaks(j)-5*jump, lens.(x.chr), x.heights(j));
        end
    end
    % sites
    if isfield(x,'sites'),
